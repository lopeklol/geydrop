<?php
    ...
    function connectToServer() {
        ...
        if (!isset($_FILES["case_image"]) || $_FILES["case_image"]["error"] == UPLOAD_ERR_NO_FILE) {
            $errorMsg = generateErrorMsg('You have to upload case image.');
            return;
        } else {
        ...
        $stmt = $connection -> prepare('INSERT INTO cases (case_name, case_value, image, for_ad, hidden) VALUES (?, ?, ?, ?, ?)');
        $stmt -> execute([$_POST['case_name'], $_POST['case_value'], $image_name, isset($_POST['for_ad']) ? true : false, isset($_POST['hidden']) ? true : false]);
...
<div id="manage_container" class="login_register_contents">
    <form action="" method="POST" class="manage_form" enctype="multipart/form-data">
        ...
        <input value="<?php print(htmlspecialchars($_POST['case_name'] ?? '')); ?>" type="text" name="case_name" minlength="3" maxlength="25" pattern="[A-Za-z0123456789' ]+" required><br>
        ...
        <input value="<?php print(htmlspecialchars($_POST['case_value'] ?? '')); ?>" type="number" step="0.01" name="case_value" min="0.00" max="99999.99" required><br>

        <label for="case_image">Case image:</label><br>
        <input type="file" name="case_image" accept=".gif, .giff, .jpg, .jpeg, .jfif, .pjpeg, .pjp, .apng, .png, .webp, .svg" required><br>
        
        <label for="for_ad">For ad (can be opened for watching an ad?):</label>
        <input <?php print(htmlspecialchars(isset($_POST['for_ad']) ? 'checked' : '')); ?> type="checkbox" name="for_ad"><br>
        
        <label for="hidden">Hidden (should be hidden and only avaliable after typing its exact name?):</label>
        <input <?php print(htmlspecialchars(isset($_POST['hidden']) ? 'checked' : '')); ?> type="checkbox" name="hidden"><br>

        <div class="g-recaptcha" data-sitekey="<?php echo $captcha_site_key; ?>"></div><br>
        
        <?php print($errorMsg); ?>
        
        <input type="submit" id="manage_submit" class="button green_button" value="Add case">
    </form>
</div>
